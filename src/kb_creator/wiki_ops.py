...
def render_kb_schema() -> str:
    ...
    return "\n".join([
        ...
        "- `distilled-note`: shared root-vault note promoted from one book-local KB.",
        ...
        "- Distilled root notes should include `book_slug`, `book_title`, `source_pages`, and `generated_from_workset`.",
        "",
        ...
        "## Workflow",
        "",
        "1. `kb build-book` normalizes one book into `raw/sources/`, archives chapters under `raw/chapters/`, and materializes one book-local KB.",
        "2. `kb distill-to-root` emits a machine-readable promotion workset from that book-local KB.",
        "3. `kb apply-root-promotion` is the single-writer step that mutates shared root-vault notes.",
        "4. Before any book-local wiki or root-vault write, validate a signed write-permit artifact.",
        "5. `kb health` checks structural integrity; `kb lint` surfaces KB-maintenance candidates, including Obsidian contract violations.",
        "6. `kb query --mode scaffold|synthesize` creates reusable answer artifacts; `--file-back yes` writes query history/index artifacts into `wiki/queries/`.",
        "",
        "## Naming Rules",
        "",
        "- Keep raw sources immutable after ingest.",
        "- Keep chapter archives under `raw/chapters/<book>/` and treat them as deterministic source artifacts.",
